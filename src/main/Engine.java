...
// TODO 
// load positions with fen on stockfish to get evaluation and check where major differences are

// TODO
// doesnt move pawn as often and doesnt gain space, also from theory rarely works
...
public class Engine {
	...
	boolean outOfTheory = false, mate = false, higherDepth = true; 
	int COLOR;
	
	long evalTime = 0, totalTime, findTime = 0, applyRevTime;
	int moveApply = 0;
	...
	int time = 3; // time available for chess.com in minutes, can only select among 1, 3 and 10 minutes
	
	int ENGINEDEPTH = 4; //TODO: enginedepth is increases when there are less pieces/less moves
	...
	private void AI() {
		...
		if (moveList.size() < 10 & !outOfTheory & time > 2) { //without false
	...
	private Move computeMove() {
		...
		if (INTERFACE == 1) {
			if (w.getTime() < time/4 & higherDepth) {
				higherDepth = false;
				ENGINEDEPTH--;
				System.out.println("higher depth from now on");
			}
		}
		...
		if (COLOR == 1)
			currentEval = findMax1(findAllLegalMoves(1), ENGINEDEPTH, Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY, startingNode); // , startingNode
		else 
			currentEval = findMin1(findAllLegalMoves(-1), ENGINEDEPTH, Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY, startingNode); // , startingNode
...
//		System.out.println("pos evaluatd: " + POSITIONS_EVALUATED);
	...
	private double findMin1(ArrayList<Move> moves, int depth, double a, double b, Node node) {
		...
		for (Move m : moves) { 
			...
			eval = minMax1(1, depth-1, a, b, nextNode);
	...
	private double findMax1(ArrayList<Move> moves, int depth, double a, double b, Node node) {
	...
	private double minMax1(int color, int depth, double a, double b, Node node) {
		...
		long t0 = System.currentTimeMillis();
		ArrayList<Move> moves = findAllLegalMoves(color);
		findTime += System.currentTimeMillis() - t0;
		
		if (moves.size() == 0) {
			// mate
			if (inCheck(color)) {
				System.out.println("mate in sight for " + -color + "depth: " + depth);
				
				return color*Double.NEGATIVE_INFINITY; 
			}
			
			// stall
			System.out.println("stall in sight for " + color + "depth: " + depth);
			...
			eval = findMax1(moves, depth, a, b, node);
		else 
			eval = findMin1(moves, depth, a, b, node);
	...
	private double findMin(ArrayList<Move> moves, int depth, double a, double b) {
		double eval, beta = Double.POSITIVE_INFINITY, valueCaptured, min = 1500; 
		Move bestMove = new Move(0, 0, 0, 0, 0);
	...
	private double findMax(ArrayList<Move> moves, int depth, double a, double b) {
		double eval, alpha = Double.NEGATIVE_INFINITY, valueCaptured, max = -1500;
		Move bestMove = new Move(0, 0, 0, 0, 0);
	...
	private double minMax(int color, int depth, double a, double b) {
		String fen = getFenNotation(board, color == 1);
		...
		if (moves.size() == 0) {
			...
			eval = findMax(moves, depth, a, b);
		else 
			eval = findMin(moves, depth, a, b);
	...
	private int[] findKings(int color) {
		int[] output = new int[4];
		
		for (int i = 0; i < 8; i++) {
			for (int j = 0; j < 8; j++) {
				if (board[i][j] == 100.0*color) {
					output[0] = j;
					output[1] = i;
				}
				else if (board[i][j] == -100.0*color) {
					output[2] = j;
					output[3] = i;
				}
			}
	...
	private ArrayList<Move> findAllLegalMoves(int color) {
		ArrayList<Move> output = new ArrayList<Move>(); 
		double valueCaptured;
		
		for (int i = 0; i < 8; i++) {
			for (int j = 0; j < 8; j++) {
				if (board[i][j]*color > 0) {
					for (Move m : getMovesAt(j, i)) {
//						valueCaptured = board[m.d][m.c];
//						applyMove(m);
						
						// TODO stop checking for checks every time, only for king moves
						// TODO shit slows down everything
//						if (!inCheck(color)) 
							output.add(m);
...
//						reverseMove(m, valueCaptured);
	...
	private boolean inCheck(int color) {
		int[] kingPos = findKings(color);
		int x = kingPos[0];
		int y = kingPos[1];
		
		// in king domain
		if ((kingPos[2] == x+1 | kingPos[2] == x-1) & (kingPos[3] == y+1 | kingPos[3] == y-1))
			return true;
		
		return attackKing(knightMoves(3*color, x, y), color) | attackKing(bishopMoves(3.2*color, x, y), color)
				| attackKing(rookMoves(5*color, x, y), color) | attackKing(queenMoves(9*color, x, y), color) 
				| attackKing(pawnMoves(color, x, y), color);
	}
	
	private boolean attackKing(ArrayList<Move> moves, int color) {
		for (Move move : moves) 
			if (move != null)
	...
	private ArrayList<Move> getMovesAt(int a, int b) {
		double v1 = board[b][a];
	...
	private ArrayList<Move> pawnMoves(int v, int a, int b) {
		ArrayList<Move> output = new ArrayList<Move>();
		
		// necessary when checking for checks
		if ((b == 7 & v == -1) | (b == 0 & v == 1))
			return output;
		
		// pawn forward
		if (board[b - v][a] == 0) {
			output.add(new Move(v, a, b, a, b - v));
				
			// double pawn jump
			if ((b == 1 & v == -1) | (b == 6 & v == 1))
				if (board[b - 2*v][a] == 0)
					output.add(new Move(v, a, b, a, b - 2*v));
		}
		
		// pawn capture
		if (a != 7)
			if (board[b - v][a+1]*v < 0)
				output.add(new Move(v, a, b, a+1, b - v));
	...
	private ArrayList<Move> knightMoves(int v, int a, int b) {
		ArrayList<Move> output = new ArrayList<Move>();
		
		if (a - 2 >= 0 & b - 1 >= 0)
			if (board[b - 1][a - 2]*v <= 0)
				output.add(new Move(v, a, b, a - 2, b - 1));
		if (a - 2 >= 0 & b + 1 < 8)
			if (board[b + 1][a - 2]*v <= 0)
				output.add(new Move(v, a, b, a - 2, b + 1));
		if (a + 2 < 8 & b - 1 >= 0)
		...
		if (b - 2 >= 0 & a - 1 >= 0)
			if (board[b - 2][a - 1]*v <= 0)
				output.add(new Move(v, a, b, a - 1, b - 2));
		if (b - 2 >= 0 & a + 1 < 8)
			if (board[b - 2][a + 1]*v <= 0)
				output.add(new Move(v, a, b, a + 1, b - 2));
		if (b + 2 < 8 & a - 1 >= 0)
	...
	private ArrayList<Move> bishopMoves(double v, int a, int b) {
		ArrayList<Move> output = new ArrayList<Move>();
	...
	private ArrayList<Move> rookMoves(int v, int a, int b) {
		ArrayList<Move> output = new ArrayList<Move>();
	...
	private ArrayList<Move> queenMoves(int v, int a, int b) {
		ArrayList<Move> output;
		
		output = rookMoves(v, a, b);
		output.addAll(bishopMoves(v, a, b));
		
		return output;
	}
	
	private ArrayList<Move> kingMoves(int v, int a, int b) {
		ArrayList<Move> output = new ArrayList<Move>();
		
		if (a > 0) {
			if (board[b][a - 1]*v <= 0)
				output.add(new Move(v, a, b, a - 1, b)); // output[0] = new Move(v, a, b, a - 1, b);
			
			if (b > 0) 
				if (board[b-1][a-1]*v <= 0)
					output.add(new Move(v, a, b, a - 1, b - 1)); // output[1] = new Move(v, a, b, a - 1, b - 1); 
			if (b < 7) 
				if (board[b+1][a-1]*v <= 0)
					output.add(new Move(v, a, b, a - 1, b + 1)); //output[2] = new Move(v, a, b, a - 1, b + 1); 
		}
		
		if (a < 7) {
			if (board[b][a+1]*v <= 0)
				output.add(new Move(v, a, b, a + 1, b));
			
			if (b > 0) 
				if (board[b-1][a+1]*v <= 0)
					output.add(new Move(v, a, b, a + 1, b - 1)); 
			if (b < 7) 
				if (board[b+1][a+1]*v <= 0)
					output.add( new Move(v, a, b, a + 1, b + 1)); 
		}
		...
		if (b > 0)
			if (board[b-1][a]*v <= 0)
				output.add(new Move(v, a, b, a, b - 1));
	...
	private boolean castlingIsPossible(Move e, double color) {
		...
		return !inCheck((int)color);
		
//		if (inCheck(e.a, e.b, (int)color)) {
